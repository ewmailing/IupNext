...
public:
  PDeleteInteractionIup(PPlot &inPPlot, Ihandle* inHandle):
    PDeleteInteraction(inPPlot), _handle(inHandle) {}
...
public:
  PSelectionInteractionIup(PPlot &inPPlot, Ihandle* inHandle):
    PSelectionInteraction(inPPlot), _handle(inHandle) {}
...
public:
  PEditInteractionIup (PPlot &inPPlot, Ihandle* inHandle):
    PEditInteraction(inPPlot), _handle(inHandle) {}
...
class PPainterIup: public Painter {
  public:
    cdCanvas *_canvas_iup;       // iup drawing surface
    cdCanvas *_canvas_db;       // double buffer drawing surface
    PPlot _plot;             // plot data holder

    int _currentDataSetIndex; // dataset index used for DS_ attributes in SetAttrubte and GetAttribute
    int _redraw; // must update the double buffer before flush

    PPainterIup(Ihandle *hnd);
    virtual ~PPainterIup();

    // Called from C functions
    void SetAttribute(char* attr, char* value);  // access from/to IUP
    char *GetAttribute(char *attr);
  ...
  protected:
    ...
    int _FontStyle, _FontSize; /* default font parameters, changed by the FONT attribute */
    Ihandle *_handle;   // IUP handle
    short int _mouseDown;
    short int _mouse_ALT;
    short int _mouse_SHIFT;
    short int _mouse_CTRL;
    char _att_buffer[128]; // buffer returned by GetAttribute
