...
static int selectedtext_cb(Ihandle *ih)
{
  ...
  Ihandle *text = IupGetHandle("text");
...
{
  if (iup_isprint(c))
    printf("K_ANY(%s, %d = %s \'%c\')\n", IupGetAttribute(IupGetParent(IupGetParent(ih)), "TITLE"), c, iupKeyEncode(c), (char)c);
  else
    printf("K_ANY(%s, %d = %s)\n", IupGetAttribute(IupGetParent(IupGetParent(ih)), "TITLE"), c, iupKeyEncode(c));
  return IUP_CONTINUE;
}
