...
iup.SetMethod("iup handle", "__tostring", iup.ihandle_tostring)
...
function iup.SetNameHandle(v)  -- used also by radio and zbox
...
function iup._ERRORMESSAGE(err,traceback)
  ...
  if (iup.error_message_popup) then
    ...
    dg:popup(iup.CENTER, iup.CENTER)
    dg:destroy()
    iup.error_message_popup = nil
  end
end
