 ...
 * 
 * Version 1.4 -- 28.03.96 (vem da versao 1.32a + BUGFIX)
 *
 * Letzte Modifikation:
 * 28.03.1996	 Corrections to be possible use 24 bits color depth.
 *		 Sorry, I don't speak Deutch.
 *		 Roberto de Beauclair Seixas - tron@icad.puc-rio.br
...
/*
** Incluido segundo instrucao do RBORGES para evitar bugs na DEC.
** Beauclair - tron@icad.puc-rio.br
** Mar/96 ...
*/
...
#include "ComboBox1P.h"

/*
** Incluido segundo instrucao do RBORGES.
** Beauclair - tron@icad.puc-rio.br
*/
#if defined(SunOS) || defined(ULTRIX)
#define ADDR_TYPE caddr_t
#else /* IRIX, AIX, Linux, OSF e VMS */
#define ADDR_TYPE XPointer
#endif
/* *** */
...
static XtResource resources[] = {
	...
	offset(combobox.VisibleItemCount), XmRImmediate, (ADDR_TYPE) 8
    }, 
	...
	offset(combobox.SelectionPolicy), XmRImmediate, (ADDR_TYPE) XmBROWSE_SELECT
    }, 
    ...
    }, 
    {
    	...
    	offset(combobox.ScrollBarDisplayPolicy), XmRImmediate, (ADDR_TYPE) XmAS_NEEDED
    },
    {
        XmNlistSizePolicy, XmCListSizePolicy, XmRListSizePolicy, sizeof(unsigned char),
        offset(combobox.ListSizePolicy), XmRImmediate, (ADDR_TYPE) XmVARIABLE
    },
    {
	...
	offset(combobox.Dummy), XmRImmediate, (ADDR_TYPE) 0
    }, 
    {
...
#if 1
LOG("## removing working procedure\n");
...
static void EditCtrlCallback (Widget w, XtPointer pClientData, XEvent *event,
                              Boolean *ContDispatch)
{
  ...
  switch (event->type)
  {
    case VisibilityNotify:
      LOG2("EditCtrlCallback: VisibilityNotify, state: %d\n", (int)event->xvisibility.state);
      /* if (cbw->combobox.ListVisible == False) */
      if (event->xvisibility.state != VisibilityUnobscured)
	ShowHideDropDownList(cbw, NULL, False);
      break;
  }
  *ContDispatch = True;
}
...
{
  LOG("## OverrideShellCallback: setting IgnoreFocusOut to TRUE\n");
                cbw->combobox.IgnoreFocusOut = True;
}
...
{
  LOG("## ArrowCrossingCallback: setting IgnoreFocusOut to TRUE\n");
                cbw->combobox.IgnoreFocusOut = True;
}
            else
{
  LOG("## ArrowCrossingCallback: setting IgnoreFocusOut to FALSE\n");
                cbw->combobox.IgnoreFocusOut = False;
}
            break;
    }
...
static void UpdateColors(XmComboBoxWidget w, int flags)
{
    Pixel  Color;
 ...
 *   neu	    aktuellster Zustand der Widget-Instanz mit
 ...
 */
    neu->combobox.Editable        = current->combobox.Editable;
    neu->combobox.ListCtrl        = current->combobox.ListCtrl;
    neu->combobox.EditCtrl        = current->combobox.EditCtrl;
    neu->combobox.LabelCtrl       = current->combobox.LabelCtrl;
    neu->combobox.SelectionPolicy = current->combobox.SelectionPolicy;
    neu->combobox.ListSizePolicy  = current->combobox.ListSizePolicy;
    neu->combobox.StaticList      = current->combobox.StaticList;
 ...
 */
    if ( current->core.sensitive != neu->core.sensitive ) {
        XtSetSensitive(neu->combobox.ListCtrl, neu->core.sensitive);
        XtSetSensitive(neu->combobox.EditCtrl, neu->core.sensitive);
        XtSetSensitive(neu->combobox.ArrowCtrl, neu->core.sensitive);
        XtSetSensitive(neu->combobox.ListCtrl, neu->core.sensitive);
        if ( !neu->core.sensitive )
            ShowHideDropDownList(neu, NULL, False);
    }
/* 
 * Die ScrollBarPolicy kann nur dann geaendert werden, wenn das Listenfeld
 * dauerhaft dargestellt wird.
 */
    if ( neu->combobox.ScrollBarDisplayPolicy != 
           current->combobox.ScrollBarDisplayPolicy )
    {
        if ( neu->combobox.StaticList )
        {
            XtVaSetValues(neu->combobox.ListCtrl,
                          XmNscrollBarDisplayPolicy, neu->combobox.ScrollBarDisplayPolicy,
                          NULL);
        }
        else
        {
...
/* Anzahl der in der Liste gleichzeitig darstellbaren Eintraege */
    if ( current->combobox.VisibleItemCount != 
           neu->combobox.VisibleItemCount ) {
	XtVaSetValues(neu->combobox.ListCtrl, 
	              XmNvisibleItemCount, neu->combobox.VisibleItemCount, 
		      NULL);
	Update = True;
    }
    if ( current->combobox.AutomaticSelection != 
           neu->combobox.AutomaticSelection )
	XtVaSetValues(neu->combobox.ListCtrl, 
	              XmNautomaticSelection, neu->combobox.AutomaticSelection, 
		      NULL);
 ...
 * benutzen ist.
 */
    if ( current->combobox.Font != neu->combobox.Font ) {
	XtVaSetValues(neu->combobox.ListCtrl, 
	              XmNfontList, neu->combobox.Font, NULL);
	XtVaSetValues(neu->combobox.EditCtrl, 
	              XmNfontList, neu->combobox.Font, NULL);
    	Update = True;
/* rborges: inserido para recalcular tamanho dos elementos apos mudanca de fonte */
	DoLayout( neu );
/* fim da mudanca */
    }
    
    Flags = 0;
    if ( neu->manager.top_shadow_color != 
         current->manager.top_shadow_color    ) Flags |= TOPSHADOWCOLOR;
    if ( neu->manager.bottom_shadow_color != 
         current->manager.bottom_shadow_color ) Flags |= BOTTOMSHADOWCOLOR;
    if ( neu->manager.foreground != 
         current->manager.foreground          ) Flags |= FOREGROUND;
    if ( neu->core.background_pixel != 
         current->core.background_pixel       ) Flags |= BACKGROUND;
    if ( Flags )
    {
      UpdateColors(neu, Flags);
      Update = True;

      neu->manager.top_shadow_color    = current->manager.top_shadow_color;
      neu->manager.bottom_shadow_color = current->manager.bottom_shadow_color;
      neu->manager.foreground          = current->manager.foreground;
      neu->core.background_pixel       = current->core.background_pixel;
    }
    
    if ( neu->combobox.ArrowCursor != current->combobox.ArrowCursor ) {
	if ( neu->combobox.ListVisible )
	    XDefineCursor(XtDisplay(neu->combobox.PopupShell), 
			    XtWindow(neu->combobox.PopupShell), 
			    neu->combobox.ArrowCursor); 
   }
 ...
 */
	for ( j = 0; j < MirrorSize; j++ ) {
		...
		case RW:   /* schreibender Zugriff erlaubt */
		    XtSetValues(MirroredResources[j].ctrl == LISTCTRL ?
			          neu->combobox.ListCtrl :  
				(MirroredResources[j].ctrl == EDITCTRL ?
				  neu->combobox.EditCtrl :
				  neu->combobox.LabelCtrl), 
			&(args[i]), 1);
		    break;
		case RWS:  /* schreibender Zugriff unter Kontrolle */
		    if ( strcmp(args[i].name, XmNvalue) == 0 ) {
			if ( neu->combobox.Editable )
			    XtSetValues(neu->combobox.EditCtrl, 
			                &(args[i]), 1);
		    }
		    break;
		case RWL: /* Transformation in andere Resource beim
			...
			if ( strcmp(args[i].name, Transformations[k].from) == 0 ) {
			    ...
			    XtSetValues(neu->combobox.LabelCtrl, 
			                &arg, 1);
			    break;
			}
		    break;
		...
		case RWI: /* Zugriff auf XmNitems */
			...
			if ( strcmp(args[k].name, XmNitemCount) == 0 ) {
			    ...
			    XtSetValues(neu->combobox.ListCtrl,
			                args, 2);
			    /*XtVaSetValues(neu->combobox.ListCtrl, 
    ...
    if ( (neu->combobox.SquareArrow != current->combobox.SquareArrow) ||
         (neu->combobox.ArrowSpacingOn != current->combobox.ArrowSpacingOn) ) {
        Update = False;
        DoLayout(neu);
    }
                                 ...
                                 Boolean Show)
{
    ...
    LOG2("ShowHideDropDownList: ListVisible: %s\n", Show == True ? "True" : "False");
...
{
  LOG("## ArrowCallback: setting IgnoreFocusOut to TRUE\n");
	        cbw->combobox.IgnoreFocusOut = True;
}
	    break;
	case XmCR_ACTIVATE:
	    LOG2("ArrowCallback: XmCR_ACTIVATE, ListVisible: %s\n", cbw->combobox.ListVisible == True ? "True" : "False");
	    XmProcessTraversal(cbw->combobox.EditCtrl, XmTRAVERSE_CURRENT);
...
LOG("## working procedure terminating\n");
                                      ...
                                      XmAnyCallbackStruct *OldInfo)
{
    ...
    info.item_position  = XmComboBoxGetSelectedPos((Widget) cbw);
			        ...
			        Dimension *pDimension)
{
	...
	RscDest.addr = (ADDR_TYPE) pDimension;
	if ( XtConvertAndStore(w, RepresentationType, &RscValue, 
			        ...
			        KeySym *pKeySym)
{
	...
	RscDest.addr = (ADDR_TYPE) pKeySym;
	if ( XtConvertAndStore(w, RepresentationType, &RscValue, 
	                       XmRKeySym, &RscDest) )
	    return True;
    }
			        ...
			        XmFontList *pFontList)
{
	...
	RscDest.addr = (ADDR_TYPE) pFontList;
	if ( XtConvertAndStore(w, RepresentationType, &RscValue, 
	                       XmRFontList, &RscDest) )
	    return True;
    }
				...
				Pixmap *pPixmap)
{
	...
	RscDest.addr = (ADDR_TYPE) pPixmap;
	if ( XtConvertAndStore(w, RepresentationType, &RscValue, 
	                       XtRBitmap, &RscDest) )
	    return True;
    }
 ...
 */
static void Initialize(Widget request, XmComboBoxWidget neu, 
 ...
 */
    XtAppAddActions(XtWidgetToApplicationContext((Widget) neu), 
	                actions, XtNumber(actions));
    
/* Allgemeine Initialisierungen... */
    neu->combobox.ConvertBitmapToPixmap            = False;
    neu->combobox.ConvertBitmapToPixmapInsensitive = False;
    
    neu->combobox.LastSelection                    = 0;
    
    neu->combobox.InInit                           = True;
 ...
 */
/* --- */
    neu->combobox.EditCtrl = XtVaCreateManagedWidget( "edit", 
	xmTextFieldWidgetClass, (Widget) neu,
	XmNverifyBell, False, 
	XmNvisual, iupmot_visual,
	...
	XmNresizeWidth, False,
        ...
        XmNbottomShadowColor, iupmot_highlight,
        XmNtopShadowColor, iupmot_shadow,
	NULL);
    XtAddCallback(neu->combobox.EditCtrl, 
...
/* Neue Translations fuer das Eingabefeld aufnehmen */
    XtOverrideTranslations(neu->combobox.EditCtrl, 
                           NewEditTranslations);
    if ( !neu->combobox.Editable ) {
	XtOverrideTranslations(neu->combobox.EditCtrl, 
                               NewEditTranslationsNE);
    ...
    XtAddEventHandler(neu->combobox.EditCtrl, VisibilityChangeMask, False,
                      (XtEventHandler) EditCtrlCallback, (XtPointer) neu);
...
/* --- */
    neu->combobox.ArrowCtrl = XtVaCreateManagedWidget("arrow", 
	xmArrowButtonWidgetClass, (Widget) neu, 
	...
	XmNcolormap, iupmot_color.colormap,
        ...
        XmNmarginHeight,        1,
	NULL);
    XmRemoveTabGroup(neu->combobox.ArrowCtrl);
    if ( neu->combobox.StaticList ) {
	XtVaSetValues(neu->combobox.ArrowCtrl, 
	    XmNmappedWhenManaged, False, NULL);
    } else {
	XtAddEventHandler(neu->combobox.ArrowCtrl, 
			  EnterWindowMask | LeaveWindowMask,
			  False, (XtEventHandler) ArrowCrossingCallback,
			  (XtPointer) neu);
	XtAddCallback(neu->combobox.ArrowCtrl, 
		      XmNactivateCallback, 
...
/* --- */
    neu->combobox.LabelCtrl = XtVaCreateWidget("label", 
	xmLabelWidgetClass, (Widget) neu, 
#ifndef Linux
	XmNstringDirection, neu->manager.string_direction, 
#endif
	XmNvisual, iupmot_visual,
        XmNdepth, iupmot_depth,
        XmNcolormap, iupmot_color.colormap,
	NULL);
    if ( neu->combobox.ShowLabel ) {
	XtManageChild((Widget) neu->combobox.LabelCtrl);
	XtAddCallback(neu->combobox.LabelCtrl, 
		      XmNhelpCallback, 
		      (XtCallbackProc) HelpCallback, 
		      (XtPointer) neu);
    }
 ...
 */
    if ( !neu->combobox.StaticList ) {
	neu->combobox.PopupShell = XtVaCreateWidget("combobox_shell", 
	    vendorShellWidgetClass, (Widget) neu, 
	    ...
	    XmNallowShellResize, True, 
            ...
            XmNcolormap, iupmot_color.colormap,
	    NULL);
	XtAddEventHandler(neu->combobox.PopupShell, 
	       EnterWindowMask | LeaveWindowMask,
     ...
     */
	neu->combobox.PopupShell = (Widget) neu;
    }
 ...
 */
    XtSetArg(args[n], XmNselectionPolicy, neu->combobox.SelectionPolicy); n++;
    ...
    XtSetArg(args[n], XmNlistSizePolicy, 
                      neu->combobox.ListSizePolicy); n++;
    XtSetArg(args[n], XmNscrollBarDisplayPolicy, 
                      neu->combobox.ScrollBarDisplayPolicy); n++;
    
    XtSetArg(args[n], XmNautomaticSelection, 
                      neu->combobox.AutomaticSelection); n++;
    XtSetArg(args[n], XmNvisibleItemCount, 
                      neu->combobox.VisibleItemCount); n++;
      XtSetArg(args[n], XmNvisual, iupmot_visual);     n++;
      XtSetArg(args[n], XmNdepth, iupmot_depth);       n++;
      XtSetArg(args[n], XmNcolormap, iupmot_color.colormap); n++;
    neu->combobox.ListCtrl = XmCreateScrolledList(
	neu->combobox.PopupShell, "list",  
	args, n);
 ...
 */
	...
	XtVaSetValues(XtParent(neu->combobox.ListCtrl), 
		      XmNtraversalOn, False, NULL);
    } else {
	if ( !neu->combobox.Editable ) {
	    XtVaSetValues(XtParent(neu->combobox.ListCtrl), 
		      XmNtraversalOn, False, NULL);
	    XmRemoveTabGroup(neu->combobox.ListCtrl);
	}
    }
    
    XtManageChild(neu->combobox.ListCtrl);
    XtAddCallback(neu->combobox.ListCtrl, 
    ...
    XtAddEventHandler(neu->combobox.ListCtrl, 
                      ButtonReleaseMask, 
		      False, (XtEventHandler) Button1UpInList, 
		      (XtPointer) neu);
    
#ifdef NODRAGNDROP
    XtOverrideTranslations(neu->combobox.ListCtrl, 
                           NewListTranslations);
#endif
    if ( neu->combobox.StaticList && neu->combobox.Editable )
        XtOverrideTranslations(neu->combobox.ListCtrl, 
                               NewListTranslationsE);
 ...
 */
    InitMirrorResources(neu);
    UpdateColors(neu, -1);
    SetValues(neu, neu, neu, wargs, ArgCount);
    
    if ( neu->combobox.ConvertBitmapToPixmap ) 
	neu->combobox.LabelPixmap = 
	    BitmapToPixmap(neu, XmNlabelPixmap, 
	               ((XmLabelRec *) neu->combobox.LabelCtrl)->
		         label.normal_GC);
    if ( neu->combobox.ConvertBitmapToPixmapInsensitive ) 
	neu->combobox.LabelInsensitivePixmap = 
	    BitmapToPixmap(neu, XmNlabelInsensitivePixmap, 
	               ((XmLabelRec *) neu->combobox.LabelCtrl)->
		         label.insensitive_GC);
    
    DefaultGeometry(neu, &width, &height, &dummy, &dummy);
    if ( neu->core.width == 0 )
	neu->core.width  = width;
    if ( neu->core.height == 0 )
	neu->core.height = height;

/*
 * Falls wir keine Fontliste besitzen, dann nehmen wir die von
 * dem Eingabefeld...
 */
    if ( neu->combobox.Font == NULL ) {
	XtVaGetValues(neu->combobox.EditCtrl, 
	              XmNfontList, &neu->combobox.Font, NULL);
	XtVaSetValues(neu->combobox.ListCtrl, 
	              XmNfontList, neu->combobox.Font, NULL);
    } else {
 ...
 */
    neu->combobox.ListVisible          = False;
    neu->combobox.IgnoreFocusOut       = False;
    neu->combobox.PendingFocusOut      = False;
    neu->combobox.PendingOverrideInOut = False;
    
    neu->combobox.PassVerification = False;
 ...
 */
static void UpdateComboBox(XmComboBoxWidget w, int Index, Boolean Deleted)
{
