...
unsigned int dimensao_limite = 2 ; /* maximum number of characters in the dimension dlg */
unsigned int titulo_limite = 20 ; /* maximum number of characters in the title dlg */
...
/* callback for entering text in the title dlg */
...
/* callback of the ok button in the title dlg */
...
void alterarcol(char* modo, int pos)
{
  ...
  else
  {
    ...
    if((((strcmp(modo,"DELCOL")==0)&&(pos == 0)&&(c == 0)))||
       (((strcmp(modo,"DELCOL")==0)&&(pos == 1)&&(c == col))))
    {
...
void alterarlin(char* modo, int pos)
{
  ...
  else
  {
    ...
    if((((strcmp(modo,"DELLIN")==0)&&(pos == 0)&&(i/col+pos == 0)))||
       (((strcmp(modo,"DELLIN")==0)&&(pos == 1)&&(i/col+pos == lin))))
    {
...
{
  if(strcmp(IupGetAttribute(matriz,"RESIZEMATRIX"), "YES") == 0)
  {
    IupSetAttribute(matriz,"RESIZEMATRIX","NO");
    IupSetAttribute(IupGetHandle("tamanho_editavel"),"VALUE","OFF");
  }
  else
  {
    IupSetAttribute(matriz,"RESIZEMATRIX","YES");
