...
{
  if (v == 1) 
    printf("FlatToggle %s - ON\n", IupGetAttribute(self, "NAME")); 
  else 
    printf("FlatToggle %s - OFF\n", IupGetAttribute(self, "NAME")); 
  return IUP_DEFAULT;
}
...
static int toggle9cb(Ihandle *self, int v)
{
  ...
  if (v == 1) printf("FlatToggle 9 - ON\n"); 
  else if (v == -1) printf("FlatToggle 9 - NOTDEF\n"); 
  else printf("FlatToggle 9 - OFF\n"); 
...
//  toggle4 = IupFlatToggle("blue foreground color");
  ...
  toggle8 = IupFlatToggle("Courier 14 Bold font");
  toggle9 = IupFlatToggle("3 State");
  
  IupSetHandle("radio test", toggle8);

  IupSetCallback(toggle1, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle2, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle3, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle4, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle5, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle6, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle8, "FLAT_ACTION", (Icallback)togglecb);
  IupSetCallback(toggle7, "FLAT_ACTION", (Icallback)toggle7cb);
  IupSetCallback(toggle9, "FLAT_ACTION", (Icallback)toggle9cb);
...
//  IupSetAttribute(box, "BGCOLOR", "255 0 0");         
  ...
  /* Dialog */
  dlg = IupDialog(box);
  IupSetAttribute (dlg, "TITLE", "IupFlatToggle Test"); 
