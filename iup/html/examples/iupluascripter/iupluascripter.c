...
char* read_file(const char* filename)
{
  ...
  lua_getfield(lcmd_state, LUA_GLOBALSINDEX, "setCurrentFile");
...
Ihandle *buildTabOutput()
{
  ...
  Ihandle *frm_consolebts = IupFrame(console_bts);
  ...
  Ihandle *output = IupVbox(frm_consolebts, ml_output, NULL);
...
Ihandle *buildTabLocals()
{
  Ihandle *list_local = IupList(NULL);
  IupSetAttribute(list_local, "EXPAND", "YES");
  IupSetAttribute(list_local, "NAME", "LIST_LOCAL");

  Ihandle *button_printLocal = IupButton("Print", NULL);
  ...
  Ihandle *vbox_local = IupVbox(button_printLocal, button_setLocal, NULL);
  IupSetAttribute(vbox_local, "EXPAND", "NO");
  IupSetAttribute(vbox_local, "MARGIN", "0x0");
  IupSetAttribute(vbox_local, "GAP", "4");

  Ihandle *frame_local = IupFrame(IupHbox(list_local, vbox_local, NULL));
  ...
  Ihandle *button_printStack = IupButton("Print ALL", NULL);
  ...
  Ihandle *vbox_stack = IupVbox(button_printLevel, button_printStack, NULL);
  IupSetAttribute(vbox_stack, "EXPAND", "NO");
  IupSetAttribute(vbox_stack, "MARGIN", "0x0");
  IupSetAttribute(vbox_stack, "GAP", "4");

  Ihandle *frame_stack = IupFrame(IupHbox(list_stack, vbox_stack, NULL));
  ...
  Ihandle *locals = IupHbox(frame_local, frame_stack, NULL);
...
Ihandle *buildTabBreaks()
{
  Ihandle *button_addbreak = IupButton("Toggle Breakpoint", NULL);
  IupSetAttribute(button_addbreak, "TIP", "Toggle breakpoint at the current line. Ctrl+B");

  Ihandle *button_removebreak = IupButton("Remove", NULL);
  IupSetAttribute(button_removebreak, "TIP", "Removes the selected breakpoint at the list.");

  Ihandle *button_removeallbreaks = IupButton("Remove All", NULL);
  IupSetAttribute(button_removeallbreaks, "TIP", "Removes all breakpoints.");

  Ihandle *hbox = IupHbox(button_addbreak, button_removebreak, button_removeallbreaks, NULL);
  ...
  Ihandle *vbox = IupVbox(hbox, list, NULL);
...
Ihandle* create_main_dialog(Ihandle *config)
{
  ...
  Ihandle *sub_menu_lua, *lua_menu, *item_debug, *item_run, *item_stop, *item_pause, *item_continue, *item_stepinto, *item_stepover, *item_stepout, *item_resetluastate;
  Ihandle *zbox_debug_continue;
  Ihandle *btn_debug, *btn_run, *btn_stop, *btn_pause, *btn_continue, *btn_stepinto, *btn_stepover, *btn_stepout, *btn_curline, *btn_curlinebreak;
  ...
  sub_menu_lua = IupSubmenu("&Lua", lua_menu);
  ...
  toolbar_hb = IupHbox(
    ...
    btn_stepover,
    btn_stepout,
    IupSetAttributes(IupFill(), "EXPAND=NO, SIZE=10"),
    btn_curline,
    btn_curlinebreak,
    IupSetAttributes(IupLabel(NULL), "SEPARATOR=VERTICAL"),
    btn_find,
  ...
  IupSetCallback(dlg, "K_F5", (Icallback)item_debug_action_cb);
  IupSetCallback(dlg, "K_cF5", (Icallback)item_run_action_cb);
  IupSetCallback(dlg, "K_sF5", (Icallback)item_stop_action_cb);
  IupSetCallback(dlg, "K_PAUSE", (Icallback)pause_action_cb);
  IupSetCallback(dlg, "K_F5", (Icallback)item_continue_action_cb);
  IupSetCallback(dlg, "K_F11", (Icallback)item_stepinto_action_cb);
  IupSetCallback(dlg, "K_F10", (Icallback)item_stepover_action_cb);
  IupSetCallback(dlg, "K_sF11", (Icallback)item_stepout_action_cb);
