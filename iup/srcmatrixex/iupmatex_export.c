...
static void iMatrixExCopyTXT(Ihandle *ih, FILE* file, int num_lin, int num_col, int skip_lin, int skip_col)
{
  ...
  int use_sort = iupAttribGetInt(ih, "EXPORTSORTED");
  ...
  for (lin = 0; lin <= num_lin; ++lin)
  {
    ...
    if (use_sort)
      sorted_lin = IupGetIntId(ih, "SORTLINEINDEX", lin);
    else
      sorted_lin = lin;

    if (iupMatrixExIsLineVisible(ih, sorted_lin))
    {
      for (col = 0; col <= num_col; ++col)
      {
        if (iupMatrixExIsColumnVisible(ih, col))
        {
          ...
          str = iupMatrixExGetCellValue(matex_data->ih, sorted_lin, col, 1);  /* get displayed value */
          if (str)
          {
            if (strchr(str, sep))
              fprintf(file, "\"%s\"", str);
...
void iupMatrixExRegisterExport(Iclass* ic)
{
  ...
  iupClassRegisterAttribute(ic, "EXPORTSORTED", NULL, NULL, NULL, NULL, IUPAF_NO_INHERIT);
